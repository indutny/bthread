...
BThread.prototype._init = function init() {
  ...
  this.resolveTxt(this.host, function(err, records) {
    ...
    if (!some) {
      ...
      self.createDNSCb = function(pass, cb) {
        if (!self.wallet) {
      ...
      self.emit('create-dns', self.createDNSCb);
      return;
    }
...
BThread.prototype._load = function _load() {
  if (--this.loadWaiting !== 0)
    return;
  this.emit('load');
  this.loaded = true;
};
...
BThread.prototype.post = function post(w, postCost, data, confirm, cb) {
  if (!this.wallet) {
    this.once('wallet', function() {
      this.post(w, postCost, data, confirm, cb);
    });
    return;
  }
  ...
  if (!this.isOwner(w)) {
    if (!data.replyTo) {
      return cb(new Error('You are not owner, please reply to ' +
                          'the existing post'));
    }
  }

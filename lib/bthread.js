...
BThread.prototype.post = function post(data, confirm, cb) {
  ...
  // Estimate fee (scripts size + signature)
  ...
  this.wallet.self.unspent().every(function(unspent) {
    // Approximate input size (signature and public key)
    byteSize += 72 + 34;

    // And update fee accordingly
    var newFee = Math.ceil(scripts.byteSize) / 1024;
    if (newFee > fee) {
      totalCost.iaddn((newFee - fee) * this.fee);
      fee = newFee;
    }
  ...
  confirm(out, fee, function(yes) {
    ...
    console.log(tx.funds.toString(10), out.toString(10), left.toString(10));
    console.log(fee, tx.render().length, byteSize);

    self.emit('log', 'Sending TX with id %s', tx.hash('hex'));
    self.pool.sendTX(tx).on('ack', function() {
      self.emit('log', 'Got acknowledge of transaction');
    });
  });
};

      ...
      if (range.end >= +new Date / 1000) {
        ...
        self.wallet.self.pending().forEach(function(tx) {
          ...
          self.pool.sendTX(tx).on('ack', function() {
            self.emit('log', 'Got ACK for TX %s', tx.hash('hex'));
          });
        });
...
BThread.prototype.post = function post(postCost, data, confirm, cb) {
  ...
  // Update output cost if needed
  if (out.cmp(cost) !== 0)
    tx.outputs[tx.outputs.length - 2].value = out;
  ...
  console.log(tx.inputs.map(function(i) {
    return {
      out: {
        hash: bcoin.utils.toHex(i.out.hash),
        index: i.out.index,
      },
      script: i.script.map(bcoin.utils.toHex)
    };
  }));
  console.log(tx.outputs.map(function(i) {
    return {
...
BThread.prototype.list = function list(hash) {
  var all = this.wallet.owner.all().map(function(tx) {
    if (hash && tx.hash('hex').indexOf(hash) === 0)
      return false;

...
BThread.prototype.post = function post(postCost, data, confirm, cb) {
  ...
  data.replyTo = replyTo && bcoin.utils.revHex(data.replyTo);
  ...
  var self = this;
  confirm(postCost.add(new bn(this.dust * scripts.length)), fee, function(yes) {
    if (!yes)
...
BThread.prototype.list = function list(hash) {
  var rhash = hash && bcoin.utils.revHex(hash)

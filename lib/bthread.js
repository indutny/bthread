...
BThread.prototype.post = function post(data, confirm, cb) {
  ...
  // Verify that `reply-to` field is present and that it refers to real TX
  if (!this.isOwner) {
    if (!data['reply-to'])
      return cb(new Error('You are not owner, please use reply-to to post'));

    var hasTX = this.wallet.owner.all().some(function(tx) {
      return tx.hash('hex') === data['reply-to'];
    });
    if (!hasTX)
      return cb(new Error('TX ' + data['reply-to'] + ' is unknown'));
  }
  ...
  var postCost = this.postCost.add(new bn((scripts.length - 1) * this.dust));

  // Use initial fee for starters
  var fee = new bn(this.fee);

  // total = cost + fee
  var totalCost = postCost.add(fee);
  ...
  function addInput(unspent, i) {
    ...
    tx.input(unspent);
    lastAdded = i;
    return tx.funds.cmp(totalCost) < 0;
  }

  // Transfer `totalCost` funds maximum
  ...
  scripts.forEach(function(script, i) {
    tx.out({
      value: i === 0 ? this.postCost : new bn(this.dust),
      script: script
    });
  }, this);

  // If not owner - add dummy output (for `left`) to calculate maximum TX size
  if (!this.isOwner)
